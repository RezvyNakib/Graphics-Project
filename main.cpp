...
// ── Page state: 0=Cover, 1=Instructions, 2=Simulation ───────
int pageNumber = 0;
...
void setOrtho2D(float w, float h) {
...
void drawCoverPage() {
    ...
    drawText("AMERICAN INTERNATIONAL UNIVERSITY-BANGLADESH", 80, 440, GLUT_BITMAP_HELVETICA_18);
    drawText("Urban Highway Motion: An Interactive Traffic Simulation", 100, 380, GLUT_BITMAP_TIMES_ROMAN_24);

    drawText("Submitted By:", 50, 300, GLUT_BITMAP_HELVETICA_12);
    drawText("1. MOHSINA AKTER LOVA      (21-44754-1)", 60, 275, GLUT_BITMAP_9_BY_15);
    drawText("2. MD SADMAN RAHMAN        (23-50211-1)", 60, 255, GLUT_BITMAP_9_BY_15);
    drawText("3. MD REZVINE ENJOY NAKIB  (23-50573-1)", 60, 235, GLUT_BITMAP_9_BY_15);
    drawText("4. FAHIMA AKTER REMU       (23-51528-1)", 60, 215, GLUT_BITMAP_9_BY_15);
    drawText("5. ANTARA BHOWMIC          (23-51952-2)", 60, 195, GLUT_BITMAP_9_BY_15);
    drawText("6. RAHAT HASAN             (23-52624-2)", 60, 175, GLUT_BITMAP_9_BY_15);

    drawText("Submitted To:",                   400, 300, GLUT_BITMAP_HELVETICA_12);
    drawText("MAHFUJUR RAHMAN",                 400, 275, GLUT_BITMAP_9_BY_15);
    drawText("Assistant Professor",             400, 255, GLUT_BITMAP_9_BY_15);
    drawText("Dept. of Computer Science",       400, 235, GLUT_BITMAP_9_BY_15);
    drawText("mahfuj@aiub.edu",                 400, 215, GLUT_BITMAP_9_BY_15);
    drawText("Submission Date: 07 May 2026",    400, 195, GLUT_BITMAP_9_BY_15);

    glColor3f(0.5f, 0.0f, 0.0f);
    drawText("Press 'N' to go to Instruction Page", 200, 65, GLUT_BITMAP_HELVETICA_18);

    glColor3f(0.1f, 0.1f, 0.6f);
    drawText("For Press N we go to Instruction Page.", 200, 42, GLUT_BITMAP_HELVETICA_12);
...
void drawInstructionPage() {
    ...
    drawText("INSTRUCTIONS", 250, 400, GLUT_BITMAP_TIMES_ROMAN_24);

    glColor3f(0.0f, 0.0f, 0.0f);
    drawText("1. Press 'N' for Day/Night Toggling (in simulation)", 100, 340, GLUT_BITMAP_HELVETICA_18);
    drawText("2. Right Mouse Button: Stop / Resume Biker",          100, 300, GLUT_BITMAP_HELVETICA_18);
    drawText("3. Birds and Fog will not stop",                      100, 260, GLUT_BITMAP_HELVETICA_18);
    drawText("4. Press 'M' to Mute Sound",                         100, 220, GLUT_BITMAP_HELVETICA_18);
    drawText("5. Press 'V' to Unmute Sound",                        100, 180, GLUT_BITMAP_HELVETICA_18);
    drawText("6. Press ESC to quit",                                100, 140, GLUT_BITMAP_HELVETICA_18);

    glColor3f(0.0f, 0.55f, 0.0f);
    drawText("Press 'N' again to START the Simulation", 170, 90, GLUT_BITMAP_HELVETICA_18);

    glColor3f(0.5f, 0.5f, 0.5f);
    drawText("Press 'B' to go back to Cover Page", 200, 50, GLUT_BITMAP_HELVETICA_12);
...
static void* audioThread(void* arg) {
    ...
    int fmt = 8; ioctl(fd, 0xC0045005, &fmt);
    int ch  = 1; ioctl(fd, 0xC0045006, &ch);
    int rate= SAMPLE_RATE; ioctl(fd, 0xC0045002, &rate);

    unsigned char buf[BUF_SAMPLES];
    int  noteIdx    = 0;
    long samplePos  = 0;
    long noteSamples= SAMPLE_RATE / 2;
...
void drawSky() {
    if (nightMode) {
        ...
        for (int i = 0; i < 120; i++) {
            float sx=(float)(rand()%W), sy=260.f+(float)(rand()%(H-260));
...
void drawGround() {
    ...
    setColor(0.22f*dim,0.55f*dim,0.18f*dim); fillRect(0,0,W,290);
    setColor(0.32f*dim,0.65f*dim,0.24f*dim);
    float g[]={0,0, (float)W,0, (float)W,215, 580,232, 280,242, 0,222};
    fillPoly(g,6);
    setColor(0.40f*dim,0.72f*dim,0.28f*dim); fillRect(0,258,W,32);
    setColor(0.48f*dim,0.62f*dim,0.28f*dim); fillRect(200,GROUND,600,18);
    setColor(0.38f*dim,0.68f*dim,0.22f*dim); fillRect(0,GROUND,200,14);
}
...
void drawRoad() {
    ...
    setColor(0.58f*dim,0.42f*dim,0.24f*dim); fillRect(0,ROAD_Y,(float)W,ROAD_H);
...
void drawOneHouse(float hx,
    ...
    float dim=nightMode?0.4f:1.0f;
    float wall_w=95.f, wall_h=88.f, cx=hx+wall_w*0.5f;
    float hy=GROUND;
...
void drawTrees() {
    float base=GROUND;
    drawTree(185,base,0.85f); drawTree(358,base,0.88f);
    drawTree(562,base,0.82f); drawTree(760,base,0.80f);
}
...
void drawBicycleAndRider(float bx, float by) {
    ...
    setColor(0.25f,0.25f,0.28f); fillCircle(bbX,bbY,4.2f);

    float hipX=stX, hipY=stY+14.f;
    float tLen=30.f, tAng=0.65f;
    float shouX=hipX+tLen*cosf(tAng), shouY=hipY+tLen*sinf(tAng);
    float handX=htX+7.f, handY=htY+12.f;
    float elbX=(shouX+handX)*0.5f+1.f, elbY=(shouY+handY)*0.5f-5.f;
    float headR=8.5f, headX=shouX+3.f, headY=shouY+12.f+headR;

    float fPedX=bbX+12.f, fPedY=bbY+3.f;
    float bPedX=bbX-12.f, bPedY=bbY-2.f;
    float fKneeX=hipX*0.40f+fPedX*0.60f+5.f, fKneeY=hipY-8.f;
    float bKneeX=hipX*0.45f+bPedX*0.55f-3.f, bKneeY=hipY-14.f;
...
void drawBiker(float offsetX) {
    drawBicycleAndRider(offsetX, ROAD_Y+22.f);
}
...
void drawHUD() {
    ...
    const char* smsg = soundOn ? "Sound: ON  [M=Mute]" : "Sound: MUTED  [V=Unmute]";
    ...
    setColor(tc*0.8f,tc*0.8f,tc*0.8f);
    glRasterPos2f(28.f,42.f);
    const char* hint="N=Day/Night  RMB=Stop Biker  M=Mute  V=Unmute  ESC=Quit";
...
void display() {
    if      (pageNumber == 0) { drawCoverPage(); }
    else if (pageNumber == 1) { drawInstructionPage(); }
    else                      { drawSimulation(); }
}
...
void keyboard(unsigned char key, int, int) {
    switch (key) {
        ...
        case 'v': case 'V':
            soundOn = 1;
#ifdef _WIN32
            sndPlaySound("nmergesound.wav", SND_ASYNC | SND_LOOP);
#endif
            break;
        case 27:
            audioRunning = 0;
...
void mouse(int button, int state, int, int) {
    if (pageNumber == 2 && button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN)
...
int main(int argc, char** argv) {
    ...
    glutInitWindowSize(800, 600);   // ← Changed from 1000x750 to 800x600
    glutInitWindowPosition(100, 80);
    glutCreateWindow("Urban Highway Motion - Traffic Simulation");
